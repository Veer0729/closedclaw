...
import { replyMd } from "./texts.ts";
import { finishOrApprove } from "./approval-session.ts";


function readOnlyConfig(): AgentConfig { // takes default config but disables everything aside able to read
...
function agentOptions(config: AgentConfig, maxSteps: number) { // a shared helper to avoid repeating functions
  return {
    model: getAgentModel(),
...
// webtool only gets addded if env key exists
// if no key return empty object
...
export async function runPlanSteps(
  ...
  for (const step of steps) {  // create a fresh agent for each step rather than one agent for the whole plan
