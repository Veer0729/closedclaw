...
export function planMessage(session: PlanSession): string {
  const lines = session.plan.steps.map((step, i) => {
    const mark = session.selected.has(step.id) ? '✅' : '⬜'; // how the selected and non selected will look
...
export function planKeyboard(session: PlanSession) { // each step gets it's own button row
...
export async function refreshPlanUi( // caled everytime a UI is clicked
