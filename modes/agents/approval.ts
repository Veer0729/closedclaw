...
function groupPending(pending: ActionLog[]): ReviewGroup[] {
  ...
  for (const a of pending) {
    if (a.type === "tool_execute") {
      shells.push(a); // shell commands go in thier own bucket, shell commands are seperate from file path
      continue;
    }
    const key = a.path;

    // everything else grouped by file path
  ...
  for (const [p, acts] of pathEntries) {
    ...
    groups.push({ label: `${p} (${kinds})`, actionId: ids, patch });
  }
...
export async function runApprovalFlow(tracker:ActionTracker): Promise<boolean> {
    ...
    if(isCancel(choice) || choice === "cancel"){
        for(const a of pending) tracker.updateStatus("id","rejected", false);
        return false
    }

    if(choice === "all"){
        for (const a in pending) tracker.updateStatus("id", "approved", true);
        return true;
    }
    ...
    for (const id of g.actionId){ // loops through every action id
        tracker.updateStatus(id, opt === "accept" ? "approved" : "rejected", opt === "accept") // if user chose accept = approved otherwise reject
    }
    break
}
    }
